...
def NUCFnlFilter(frames, Ts=15, Tg=11, offset_only = True):
    all_frames_est = []
    coeffs = init_nuc()
    for i in tqdm(len(frames[1:]), desc="NUCFnlFilter algorithm", unit="frame"):
        if M_n(frame=frames[i], frame_n_1=frames[i-1], Ts=Ts , Tg=Tg):
            frame_est = Xest(g=coeffs['g'], y=frames[i], o=coeffs['o'])
                ...
                frame_est , coeffs['g'], coeffs['o']= NUCFnlFilter_frame(frames[i], coeffs, N=i, offset_only=offset_only)
        all_frames_est.append(frame_est)
    return np.array(all_frames_est, dtype=frames.dtype)

def NUCFnlFilter_frame(frame, frame_n_1, coeffs, Eij_n_1, K, N:int, k_size=3, offset_only=True):
    X_est = Xest(g=coeffs['g'], y=frame, o=coeffs['o'])
    delta = e(frame, X_est, coeffs, Eij_n_1)
    Eij = delta**2
...
def e(frame, X_est, coeffs, Eij_n_1, beta= 2.42):
